...
**A publication-ready toolkit for cyclic voltammetry (CV) recorded at multiple scan rates — seven complementary analyses sharing one uploaded file.**
...
Seven scan-rate CV analyses, each in its own tab, all reading the **same uploaded file**:

| Analysis | What it extracts | Regime |
| --- | --- | --- |
| **Dunn's Method** | Power-law *b*-value charge-storage classification + capacitive/diffusive current split | any |
| **Trasatti Analysis** | Inner / outer / total voltammetric charge (site accessibility) | pseudocapacitive / porous |
| **ECASA** | Double-layer capacitance C<sub>dl</sub> → electrochemically active surface area | non-Faradaic |
| **Areal Capacitance** | Areal capacitance C<sub>A</sub> (F cm⁻²) + rate-capability / retention | capacitive |
| **Randles–Sevcik** | Diffusion coefficient D from peak current vs √(scan rate) | Faradaic |
| **Peak Area / Charge** | Charge Q, surface coverage Γ, moles N, Q_a/Q_c ratio | Faradaic |
| **Electron-Transfer Kinetics** | Heterogeneous rate constant k⁰ (Nicholson) / α, k_s (Laviron) | Faradaic |

Tab order: **Dunn → Trasatti → ECASA → Areal → Randles–Sevcik → Peak Area → Electron-Transfer Kinetics**, each grouped beside its closest companion. Faradaic tabs gate on genuine redox-peak detection and refuse to run on non-Faradaic data.

Available in **three forms**, all with the same analyses:

1. **Hosted web demo (Streamlit)** — fastest way in, nothing to install: **[electrosim-cvanalysis-suite.streamlit.app](https://electrosim-cvanalysis-suite.streamlit.app/)**
...
**Web demo:** open **<https://electrosim-cvanalysis-suite.streamlit.app/>**, upload a CV file (wide Excel or stacked CSV), get all seven analyses instantly.

**Desktop install (Windows 64-bit):** download **[ElectroSim-CVAnalysis-Suite-Setup-2.1.exe](../../releases/latest)** from the [Releases](../../releases) page → double-click → 3 screens (Welcome → Installing → Finish, ~15 s, no admin rights). Inside the app, click **Load files…** → pick a template from `sample_data/` → switch to the analysis tab you need.
...
`Setup-2.1.exe` is unsigned, so Windows SmartScreen shows "Windows protected your PC" the first time — click **More info → Run anyway**. To confirm the file is genuine, check its SHA-256:

```powershell
Get-FileHash -Algorithm SHA256 $env:USERPROFILE\Downloads\ElectroSim-CVAnalysis-Suite-Setup-2.1.exe
```

Expected (case-insensitive): `10A0DB7F8BCAC53C92206BBA28C99CA89FF6B4D9761043EB049F5265A6FC1294`
...
py -3.12 -m streamlit run electrosim_app.py    # cp312 engine
py -3.14 -m streamlit run electrosim_app.py    # cp314 engine
...
> End users don't need this — they download the prebuilt installer. The analysis source is **not** in this public repo; the maintainer builds from a private tree. Building in a clone of the public repo fails with "module not found" — expected.

**Build reference files** (tracked in [`release/build/`](release/build/)):

| File | What it is |
| --- | --- |
| `ElectroSim-CVAnalysis-Suite.spec` | PyInstaller config for the **PyWebView/Streamlit hybrid** build (main app) |
| `ElectroSim-CVAnalysis-Suite-Tkinter.spec` | PyInstaller config for the **Tkinter** fallback build |
| `electrosim_build.ps1` | PowerShell wrapper running PyInstaller against both specs (`-Target streamlit\|tkinter\|all`, optional `-Clean`, `-SyncDeps`) |

The hybrid spec freezes the Streamlit launcher and the seven-analysis app; the Tkinter spec freezes the native Tkinter GUI and its analysis module. The hosted-demo engine (`release/web-demo/electrosim_engine.cp3XX.pyc`) is compiled separately — see *How the web app is packaged* above.
...
#    (-SyncDeps first installs scipy/plotly/kaleido/openpyxl/pywebview/... from requirements.txt)
.\release\build\electrosim_build.ps1 -Clean -SyncDeps
